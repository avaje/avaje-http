...
class NimaPlatformAdapter implements PlatformAdapter {
  ...
  public void writeReadParameter(Append writer, ParamType paramType, String paramName) {
    switch (paramType) {
      case PATHPARAM -> writer.append("pathParams.contains(\"%s\") ? pathParams.get(\"%s\") : null", paramName);

      case QUERYPARAM -> writer.append("req.query().contains(\"%s\") ? req.query().get(\"%s\") : null", paramName);

      case FORMPARAM -> writer.append("formParams.contains(\"%s\") ? formParams.get(\"%s\") : null", paramName);

      case HEADER -> writer.append(
          "req.headers().value(HeaderNames.create(\"%s\")).orElse(null)", paramName);

      case COOKIE -> writer.append("req.headers().cookies().contains(\"%s\") ? req.headers().cookies().get(\"%s\") : null", paramName);
  ...
  public void writeReadParameter(Append writer, ParamType paramType, String paramName, String paramDefault) {
    switch (paramType) {
      case PATHPARAM -> writer.append(
          "pathParams.contains(\"%s\") ? pathParams.get(\"%s\") : \"%s\"", paramName, paramDefault);

      case QUERYPARAM -> writer.append(
          "req.query().contains(\"%s\") ? req.query().get(\"%s\") : \"%s\"", paramName, paramDefault);

      case FORMPARAM -> writer.append(
          "formParams.contains(\"%s\") ? formParams.get(\"%s\") : \"%s\"", paramName, paramDefault);

      case HEADER -> writer.append(
          "req.headers().value(Http.Header.create(\"%s\").orElse(\"%s\")", paramName, paramDefault);

      case COOKIE -> writer.append(
          "req.headers().cookies().contains(\"%s\") ? req.headers().cookies().get(\"%s\") : \"%s\"", paramName, paramDefault);
